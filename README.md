...
- **会主动回想**：内置动作「回想」让她自己想翻记忆——按主题（「想想在厨房做饭的事」）、按地点、或按整个区域（「回忆公园里的事」）。翻完带着结果再开口一次；什么都没想起来时照实说，不编。
...
- **像人一样说话**：一次说几句时，两条之间会按字数停一下再发（打字停顿，可配置或关闭）；
  群里聊天记录按原样一条时间线给她看，她能分清「有人在跟我说话」和「群里在聊，我插一句」。
- **看得懂她在干嘛**：实时状态页的左栏分成「时间与日程 / 正在进行 / 互动与运行」三块——下一条日程还有多久、
  当前动作与计划的进度条、她现在在哪个区域哪个地点、谁刚说过话、本小时还剩多少额度、
  最近一次模型调用和发送通道好不好；右栏是她的推理草稿（在哪 / 什么状态 / 在和谁说话 / 打算怎么做）
  和可读的当前计划，外加"某个动作为什么被跳过"；日志页按时间线还原她的每一次决定。
